...
export default function SpellingSession({ list, onExit }) {
  ...
  function moveNext(retryCurrent = false) { const remaining = queue.slice(1); const nextRetries = retryCurrent ? [...retryQueue, word] : retryQueue; setQueue(remaining.length ? remaining : nextRetries); setRetryQueue(remaining.length ? nextRetries : []); setAnswer(''); setLastAnswer(''); setPhase('prompt') }
